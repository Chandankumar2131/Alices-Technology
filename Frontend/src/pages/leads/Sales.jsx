...
export default function Sales() {
  ...
  const [editing, setEditing] = useState(null); const [form, setForm] = useState({ salesComment: "", status: "Follow-UP", callDate: "", callTime: "" });
  ...
  const edit = (row) => { setEditing(row); setForm({ salesComment: row.salesComment || "", status: LEAD_STATUSES.includes(row.status) ? row.status : "Follow-UP", callDate: row.callDate ? String(row.callDate).slice(0, 10) : "", callTime: row.callTime || "" }); };
  ...
  return <div className="space-y-6">
    ...
    <div className="grid grid-cols-1 gap-3 sm:grid-cols-3"><StatCard label="Assigned Leads" value={rows.length} /><StatCard label="Follow-Ups" value={rows.filter((row) => row.status === "Follow-UP").length} /><StatCard label="Closed" value={rows.filter((row) => row.status === "Closed").length} /></div>
