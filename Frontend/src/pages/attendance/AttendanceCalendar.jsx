...
export default function AttendanceCalendar({ calendar = [], loading, month, year }) {
  ...
  if (!calendar.length) return <p className="text-sm text-slate-400">No data for this month.</p>;
  ...
  return (
    <div>
      <div className="overflow-x-auto pb-1">
        <div className="mb-2 grid min-w-[42rem] grid-cols-7 gap-2 text-center text-xs font-semibold text-slate-400">
          {WEEKDAYS.map((day) => (
            <div key={day}>{day}</div>
          ))}
        </div>
          ...
          {calendar.map((day) => {
            ...
            return (
              ...
              >
                ...
                {day.checkIn && (
                  <p className={`mt-1 text-[10px] ${visibleStatus === "Present" ? "text-emerald-700" : "text-slate-400"}`}>
