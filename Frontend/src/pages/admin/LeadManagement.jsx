...
export default function LeadManagement() {
  ...
  return <div className="space-y-6"><div><h1 className="text-2xl font-bold text-slate-50">Lead & Sales Analysis</h1><p className="mt-1 text-sm text-slate-400">Analyze daily Lead Generation and Sales activity.</p></div>
    <div className="grid grid-cols-2 gap-3 lg:grid-cols-4"><StatCard label="Leads Generated" value={rows.length} /><StatCard label="Sales Updates" value={activities.length} /><StatCard label="Follow Ups" value={activities.filter((row) => row.status === "Follow Up").length} /><StatCard label="Converted" value={activities.filter((row) => row.status === "Converted").length} /></div>
