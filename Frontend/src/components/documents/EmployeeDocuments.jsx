...
const DOCUMENT_TYPES = ["Aadhaar Card", "PAN Card", "Passport", "Driving Licence", "Educational Certificate", "Experience Letter", "Offer / Appointment Letter", "Relieving Letter", "Address Proof", "Other"];
...
export default function EmployeeDocuments({ admin = false, employeeId = "" }) {
  ...
  const upload = async (event) => {
    ...
    setBusy("upload");
    try {
      await employeeDocumentService.upload({ documentType: form.documentType, documentNumber: form.documentNumber, dataUrl: await readDataUrl(form.file), fileName: form.file.name, mimeType: form.file.type, size: form.file.size });
      notify.success("Document uploaded for review"); setForm({ documentType: "Aadhaar Card", documentNumber: "", file: null }); await load();
    } catch (error) { notify.error(errorText(error)); }
    finally { setBusy(""); }
  };
